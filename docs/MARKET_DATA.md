...
- 시장 하나당 GeoJSON 파일 하나를 기본으로 사용합니다.
- `packages/market-data`를 모바일, API와 관리자 화면이 함께 쓰는 유일한 원본으로 둡니다.
...
당장은 파일을 삭제하거나 이동하지 않습니다. 먼저 `packages/market-data`와 변환·검증 테스트를 만든 뒤 다음 순서로 한 번만 이관합니다.
...
6. 패키지 어댑터가 기존 Naver Map 화면과 동일한 경계를 만드는지 비교합니다.
7. 모바일 import와 지오펜스 테스트가 통과한 뒤 루트와 모바일의 레거시 JSON을 같은 커밋에서 제거합니다.
...
파일명과 `Feature.id`는 소문자 kebab-case의 안정된 ID를 사용합니다. 시장 이름이 바뀌더라도 기존 사용자 진행도와 API 참조가 깨지지 않도록 ID는 변경하지 않습니다.

## 표준 저장 형식

원본은 [RFC 7946 GeoJSON](https://www.rfc-editor.org/info/rfc7946/)의 `Feature`로 저장합니다. 하나로 이어진 시장은 `Polygon`, 서로 떨어진 구역을 하나의 시장으로 관리해야 하면 `MultiPolygon`을 사용합니다.
...
검증에 실패한 데이터는 앱 번들과 API 배포물에 포함하지 않습니다.

## 앱과 API에서 사용하는 방법

`packages/market-data`는 원본 GeoJSON과 함께 다음 읽기 계약을 제공합니다.

- 대전 지역의 활성 시장 목록 조회
- 안정된 ID로 시장 조회
- 시장 중심점 또는 bounding box 조회
- GeoJSON 경계를 Naver Map Polygon 좌표로 변환
- 지오펜스 패키지가 사용할 `Polygon | MultiPolygon` 경계 제공

MVP에서는 검증된 시장 데이터를 모바일에 번들합니다. 이후 운영 중 경계 갱신이 필요하면 API가 최신 revision을 제공하고 모바일은 검증된 로컬 데이터를 캐시합니다. 리워드 발급 시에는 API가 자신이 보유한 동일 revision의 경계로 최종 판정합니다.
